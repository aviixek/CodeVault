...
<head>
  ...
  <style>
    ...
    * {
    ...
    .lang-java { background: rgba(59, 130, 246, 0.1); color: #60a5fa; border: 1px solid rgba(59, 130, 246, 0.2); }
    .lang-mysql { background: rgba(245, 158, 11, 0.1); color: #fbbf24; border: 1px solid rgba(245, 158, 11, 0.2); }
    .lang-jsp { background: rgba(16, 185, 129, 0.1); color: #34d399; border: 1px solid rgba(16, 185, 129, 0.2); }
    .lang-python { background: rgba(139, 92, 246, 0.1); color: #a78bfa; border: 1px solid rgba(139, 92, 246, 0.2); }
    .lang-javascript { background: rgba(236, 72, 153, 0.1); color: #f472b6; border: 1px solid rgba(236, 72, 153, 0.2); }
    .lang-other { background: rgba(107, 114, 128, 0.1); color: #9ca3af; border: 1px solid rgba(107, 114, 128, 0.2); }
    ...
    .empty-icon {
      color: var(--text-tertiary);
      margin-bottom: 24px;
      filter: drop-shadow(0 0 10px rgba(var(--accent-blue-rgb), 0.15));
    }
...
<body>
  ...
  <!-- ==========================================
       GLOBAL NAVBAR (AUTHENTICATED STATE)
       ========================================== -->
  <header class="navbar">
    <div class="navbar-container">
      <a href="${pageContext.request.contextPath}/dashboard.jsp" class="brand">
        <!-- Logo SVG -->
      ...
      <nav class="nav-menu">
        ...
        <a href="${pageContext.request.contextPath}/addSnippet.jsp" class="nav-link">Add Snippet</a>
  ...
  <main class="app-main dashboard-main">
    <div class="dashboard-header fade-in">
      <h1 class="welcome-title">Welcome back, <span>${not empty sessionScope.username ? sessionScope.username : 'Developer'}</span></h1>
    ...
    <div class="stats-grid fade-in delay-1">
      <div class="glass-card stat-card">
        ...
        <span class="stat-value">${not empty snippets ? snippets.size() : 0}</span>
      </div>
    ...
    <div class="dashboard-controls fade-in delay-2">
      ...
      <a href="${pageContext.request.contextPath}/addSnippet.jsp" class="btn btn-primary btn-add-snippet">
          ...
          <c:forEach var="snippet" items="${snippets}">
            <div class="glass-card snippet-card">
              ...
              <div class="snippet-footer">
                <span class="snippet-date">Added on: ${snippet.createdAt}</span>
                
                <div class="snippet-actions">
                  <a href="${pageContext.request.contextPath}/editSnippet?id=${snippet.id}" class="btn-action" title="Edit Snippet">
                  ...
                  <form action="${pageContext.request.contextPath}/deleteSnippet" method="post" onsubmit="return confirm('Are you sure you want to delete this snippet?');">
                    <input type="hidden" name="id" value="${snippet.id}" />
                    <button type="submit" class="btn-action btn-action-delete" title="Delete Snippet">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <p class="empty-description">
            <c:choose>
              <c:when test="${not empty param.query}">
                Your search query "${param.query}" did not match any stored snippets. Try another search.
              </c:when>
              <c:otherwise>
                Your code library is currently empty. Get started by creating your very first programming snippet!
              </c:otherwise>
            </c:choose>
          </p>
          <a href="${pageContext.request.contextPath}/addSnippet.jsp" class="btn btn-primary">Create Snippet</a>
        </div>
      </c:otherwise>
    </c:choose>
  </main>
  ...
  <script>
    ...
    document.querySelectorAll('.glass-card, .snippet-card').forEach(card => {
